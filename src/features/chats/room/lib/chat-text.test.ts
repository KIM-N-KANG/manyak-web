...
describe('parseChatSegments', () => {
  ...
  it('handles multiple emphasis regions on one line', () => {
    expect(parseChatSegments('*A* 그리고 *B*')).toEqual([
  ...
  it('returns plain text when there is no marker', () => {
    expect(parseChatSegments('일반 문장')).toEqual([
