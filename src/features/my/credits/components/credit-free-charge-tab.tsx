...
import { buildAttendanceTitleLines, CREDIT_CHARGE_COPY } from '../constants';
    ...
    <div className="flex h-full scroll-fade-b flex-col gap-8 overflow-y-auto overscroll-contain pt-4 pb-2">
      <section className="mx-4 flex flex-col gap-4 rounded-lg bg-muted p-4">
        <h2
          className={cn(
            'text-lg font-bold',
            attendanceReward === undefined && 'animate-pulse',
          )}>
          {buildAttendanceTitleLines(formatCreditAmount(attendanceReward)).map(
            (line) => (
