...
test('orval config reads backend OpenAPI and generates API plus Zod outputs', () => {
  ...
  assert.match(source, /target:\s*'src\/api\/generated\/api\/index\.ts'/);
  assert.match(source, /schemas:\s*'src\/api\/generated\/api\/model'/);
...
test('generated clients call the custom mutator with URL and request init', () => {
  const apiFiles = listFiles('src/api/generated/api').filter((path) =>
    path.endsWith('.ts'),
  );
